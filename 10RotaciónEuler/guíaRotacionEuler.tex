...
\begin{enumerate}
	...
	Resultado:
	\(
		\left\{ A_{X} : 0, \  A_{Y} : \frac{I_{y} \omega^{2} \sin{\left(\theta \right)} \cos{\left(\theta \right)}}{d_{GA} + d_{GB}} - \frac{I_{z} \omega^{2} \sin{\left(\theta \right)} \cos{\left(\theta \right)}}{d_{GA} + d_{GB}} + \frac{d_{GB} g m}{d_{GA} + d_{GB}}, \  A_{Z} : 0\right\}
	\)

	\(
		\left\{ B_{X} : 0, \  B_{Y} : - \frac{I_{y} \omega^{2} \sin{\left(\theta \right)} \cos{\left(\theta \right)}}{d_{GA} + d_{GB}} + \frac{I_{z} \omega^{2} \sin{\left(\theta \right)} \cos{\left(\theta \right)}}{d_{GA} + d_{GB}} + \frac{d_{GA} g m}{d_{GA} + d_{GB}}\right\}
	\)
	\(
	\left\{ A_{X} : 0, \  A_{Y} : 71.6, \  A_{Z} : 0, \  B_{X} : 0, \  B_{Y} : 26.5\right\}
	\)
	...
	\item 
	\begin{minipage}[t][1cm]{0.65\textwidth}
	...
	\end{minipage}
	\begin{minipage}[c][5cm][t]{0.3\textwidth}
		\includegraphics[width=\textwidth]{hibb_21-45_axis2}
	\end{minipage}
		Resultado: \(
			\omega = \frac{3}{2} \sqrt{\frac{6 g}{L \left(- 9 \sin^{2}{\left(\theta \right)} - 15 \cos{\left(\theta \right)} + 13\right)}} \sin{\left(\theta \right)}			% \omega = \sqrt{\frac{3 g }{L \cos(\theta)}}
			% \omega = \sqrt{\frac{3 g \cos(\theta)}{L \sin^2(\theta)}}
		\)
		.
	Se sugieren los siguientes pasos:
	\begin{enumerate}
		\item Obtener el tensor de inercia en los ejes de la barra (sistema azul).
		\item Sumar el tensor de Steiner de un desplazamiento $L/2$ en el eje longitudinal para calcular el tensor de inercia en A.
		\item Rotar en \(\theta\) para obtener el tensor de inercia en ejes paralelos al sistema rojo pero con origen en $A$.
		\item Sumar el tensor de Steiner para obtener el tensor calculado en el origen del sistema rojo.
		\item El desplazamiento entre los origenes de ambos sistemas es el que debe consirerarse para el torque que produce el peso de la barra.
	\end{enumerate}
	...
	\item 
	\begin{minipage}[t][6.3cm]{0.65\textwidth}
	...
	No olvide incluir en las ecuaciones de la segunda ley de Newton la aceleración tangencial del centro de masa \(G\) del cilindro en función de \(\alpha_c\).
	\begin{enumerate}
		\item Determine las reacciones que deben proveer los cojinetes.\\
		Resultado: \(
			\left\{ A_{y} : \frac{\alpha_{c} m \left(3 R^{2} + d_{AG}^{2} - 10 d_{AG} d_{GB} + d_{GB}^{2}\right)}{12 \left(d_{AG} + d_{GB}\right)}, \  A_{z} : \frac{m \left(3 R^{2} \alpha_{c} \omega t - 13 \alpha_{c} d_{AG}^{2} \omega t - 2 \alpha_{c} d_{AG} d_{GB} \omega t - \alpha_{c} d_{GB}^{2} \omega t + 12 d_{GB} g\right)}{12 \left(d_{AG} + d_{GB}\right)}\right\}
		\)
		\(
			\left\{ B_{x} : \frac{\alpha_{c} m}{d_{AG}}, \  B_{y} : \frac{\alpha_{c} m \left(- 3 R^{2} - 13 d_{AG}^{2} - 2 d_{AG} d_{GB} - d_{GB}^{2}\right)}{12 \left(d_{AG} + d_{GB}\right)}, \  B_{z} : \frac{m \left(- 3 R^{2} \alpha_{c} \omega t + 13 \alpha_{c} d_{AG}^{2} \omega t + 2 \alpha_{c} d_{AG} d_{GB} \omega t + \alpha_{c} d_{GB}^{2} \omega t + 12 d_{AG} g\right)}{12 \left(d_{AG} + d_{GB}\right)}\right\}
		\)
		\item Convierta los datos en unidades imperiales (pies, libras) en unidades del Sistema Internacional para obtener las reacciones en Newton.
		
		Resultado: \(
			\left\{ A_{y} : -7.52, \  A_{z} : 33.4 - 63.3 t, \  B_{x} : 268.0, \  B_{y} : -17.4, \  B_{z} : 63.3 t + 33.4\right\}
	...
	\item 
	\begin{minipage}[t][4.55cm]{0.65\textwidth}
