...
\begin{enumerate}
	...
	\begin{minipage}[t][4.5cm]{0.62\textwidth}
		...
		Esto impone que \(\rho = R\) y \(\dot{\rho} \equiv 0\).

		Para obtener \(\theta^\mathrm{despegue}\) hay que tener presente que:
		\begin{itemize}
			\item Cuando la fuerza de ligadura para la coordenada radial, \(Q_{\rho}\), sea nula es el momento en que se produce el despegue.
			\item Al no haber rozamiento la energía mecánica (cinética + potencial) es constante.
		\end{itemize}
	\end{minipage}
	...
	\begin{enumerate}
		...
		\item Escriba una ecuación para igualar la energía mecánica inicial con la de despegue. Para esto tendrá que substituir en su expresión general una vez para las condiciones iniciales (cuando estaba arriba) y otra vez para el momento del despegue.
		\item Con la ecuación que iguala las energías mecánicas y con la de \(Q_{\rho}= 0\) (al momento del despegue) podrá calcular el ángulo en que la partícula se despega de la semiesfera.
