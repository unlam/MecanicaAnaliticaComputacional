...
\input{../referencia/figurasLaTeX/introUNLaM.tex}
...
Mientras gira manteniendo su horizontalidad una araña que estaba originalmente en su centro camina en dirección radial con $v_a$ constante.

\textbf{Determine}:
\begin{enumerate}[label=\alph*)]
	\item Velocidad y aceleración de la araña consideradas desde un sistema de coordenadas fijas al suelo.
...
\textbf{Determine}:
\begin{enumerate}[label=\alph*)]
\item Una expresión para la aceleración de la masa $m$ en el sistema de referencia inercial $O_{xyz}$.
...
\item
\textbf{Perno engarzado}\\
\begin{minipage}[t][3.5cm]{0.65\textwidth}
El perno $B$ está engarzado a una guía recta $\overline{AC}$ y a otra circular de radio $R$ centrada en $O$.
Esta última está fija, pero la guía recta gira con velocidad angular constante $\Omega$ en torno del punto $A$ ubicado a una distancia $R$ a izquierda de $O$ (en el croquis).
De ese punto $A$ el perno está a una distancia $\rho$.
La longitud de la guía recta $\overline{AC}$ habilita que el perno recorra la guía circular entre $D$ y $E$ sin restricciones.

\textbf{Determine}: $\dot{\vec{r}}$ y $\ddot{\vec{r}}$ del perno.
\end{minipage}
\begin{minipage}[c][1.5cm][t]{0.27\textwidth}
	\includegraphics[width=\textwidth]{\detokenize{guiaNoInercial_ej3}}
\end{minipage}
...
\item
\textbf{Extensión de plumas de un satélite}\\
...
Dos instrumentos científicos $P$ están originalmente a $R= \SI{1,2}{\metre}$ de tal eje, pero conectados a plumas radialmente extensibles.
La posterior extensión de su longitud $l$ se produce a una tasa constante hasta alcanzar los \SI{3}{\metre}.
Durante tal procedimiento opera un mecanismo interno que mantiene constante la velocidad de rotación del satélite.
La máxima aceleración que estos instrumentos soportan es \SI{0,011}{\metre\per\second\squared}.

Pregunta conceptual:
Asumiendo que el momento angular del satélite se calcula con respecto al centro de masa del satélite.
¿Cambia este como resultado de la extensión de las plumas que soportan los instrumentos?
\medskip
\textbf{Determine}: Máxima tasa de extensión de las barras.
...
Pregunta conceptual:
¿Qué causa en este sistema un $\dv{\vec{p}}{t} \neq 0$?\\
\begin{enumerate*}[label=\alph*),itemjoin={,\quad}]
	\item $\SI{0}{\newton}$
	\item $M g$
